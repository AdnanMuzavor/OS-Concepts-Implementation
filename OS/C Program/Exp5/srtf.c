...
void SRTF(Prc *p,int first_p,int n)
{
    ...
    for(time=0; count!=n; time++)
    {
        ...
        for(i=0; i<n; i++)
        {
            ...
            if(p[i].a_time<=time && p[i].rem_btime!=0)
            {
                ...
                ///If this process has lesser remaining burst time
                else if(p[i].rem_btime<p[select_process].rem_btime) select_process=i;
            }
...
int main()
{
    ...
    input(p1,pr_order,n);
    printf("\nAfter Shortest Remaining Time First Schedulling: ");
    printf("\n");
    SRTF(p1,pr_order,n);
    return 0;
}
