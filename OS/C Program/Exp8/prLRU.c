///Page replacement Least Recently Used Alogorithm
...
int Give_index(int key,int start,int *pages)
{
    int i;
    for(i=start; i>=0; i--)
    {
...
{
    printf("\n__Applying Page Replacement Least Recently Used Algorithm__");
    printf("\n------------------------\n");
    printf(" Page  |    Frame");
    printf("\n------------------------");
    int inserter=0,i,faults=0,j;
    int lrf_index=MAX,index_to_insert=-1;
    printf("\n");
    for(i=0; i<n; i++)
    {
        if(!Check_if_in(fsz,frame,pgs[i]))
        {
            int lrf_index=MAX,index_to_insert=-1,flag_direct=0;
            for(j=0; j<fsz; j++)
            {
                ...
                int index=Give_index(frame[j],i,pgs);
