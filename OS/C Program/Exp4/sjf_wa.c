#include<stdio.h>
///Non-Arrival Shortest job first
...
void merge(int l,int m,int h,Prc p1[],int order[])
{
    ...
    while(i<=m && j<=h)
    {
        ///if sorting based on B_time directly s_key=0

            if(p1[i].b_time<=p1[j].b_time)
            {
                temp[k]=p1[i].b_time;
                new_order[k]=order[i];
                i++;
...
void SJF(Prc p1[],int *order,int n)
{
...
int main()
{
    ...
    for(i=0; i<n; i++)
    {
        printf(" p%d \t\t%d\n",pr_order[i]+1,p1[i].b_time);
    }
    mergesort(0,n-1,p1,pr_order); ///Sort based on burst time only
    printf("\nAfter Shortest Job First Schedulling\n");
    SJF(p1,pr_order,n);
    return 0;
}
