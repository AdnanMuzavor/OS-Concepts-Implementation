#include<stdio.h>
///With Arrival Shortest job first
...
void raw_display(int order[],Prc p1[],int n)
{
    ...
    for(i=0; i<n; i++)
    {
        printf(" p%d \t\t%d\t\t\t%d\n",order[i],p1[i].a_time,p1[i].b_time);
    }
}
...
int decide_first(Prc p1[],int processed[],int n,int prev_burst_time)
{
    ...
    int first_p=-1,min_arrival_time=1000,min_burst_time=1000,i=0;
...
void SJF_A(Prc p1[],int order[],int n)
{
...
int main()
{
    ...
    printf("\nAfter Shotrest Job First Schedulling:\n");
    SJF_A(p1,pr_order,n);
    return 0;
}
